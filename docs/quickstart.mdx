...
import taskListSync from '../static/img/task-list-sync.gif'
import textMerge from '../static/img/text-merge-KleppmannStrangeLoop2023.png'
import architecture from '../static/img/automerge-arch-KleppmannStrangeLoop2023.png'
...
<Figure alt="Screen capture of the non-syncing app" caption="The (unimpressive) app in its starting state, before you give it superpowers with Automerge" />
...
<Figure  
  src={architecture}
  alt='TODO diagram of automerge project components'
  caption='TODO Placeholder - Automerge system diagram from  "New algorithms for collaborative text editing" by Martin Kleppmann (Strange Loop 2023)'
...
So instead of reassigning string values, use the `updateText()` function from `@automerge/automerge` to [change string values more efficiently](/docs/documents/text/), with additional advantages such as the ability to merge substring-level edits.
...
const renamed = updateText( 
  list.tasks[0], // the (sub) document to update - TODO 
  ['title'], // array representing the path to the text to update - TODO
...
Pass Automerge two documents with different changes, and it will return a new document that includes the changes from both:

```ts
import { updateText, merge } from '@automerge/automerge/next'

const more = updateText(list.tasks[0], ['title'], 'Learn more Automerge');
const write = updateText(list.tasks[0], ['title'], 'Write Automerge')

// TODO @vakila syntax
const merged = merge([more, write]); 
