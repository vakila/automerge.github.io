...
In App.tsx, import the `@automerge/automerge` package, then use the data types it exposes to define the shape of the document(s) your app will need. 

For example, for a todo-list app, you might represent each item as a `TodoDoc` with a string that describes the `task`, and a boolean `done` property. 
...
For example, to mark a `todo` as completed, you can set its `done` value like so:

```ts
const doneTodo = A.change(todo, d => d.done = true);
```

Or to change the name of the task: 

```ts
const renamedTodo = A.change(todo, d => d.task = 'Learn more Automerge');
```
...
```ts
const mergedTodo = A.merge(doneTodo, renamedTodo);
console.log(mergedTodo); // { task: 'Learn more Automerge', done: true }
```
