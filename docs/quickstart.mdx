...
- [Change](/docs/documents/conflicts/) documents' data and [merge](/docs/documents/conflicts/) changes from different peers
...
Each document is identified by a [Document URL](../concepts/#document-urls) that serves as its unique identifier, e.g.:
...
As shown above, you can manipulate documents directly with `A`, which is useful for understanding Automerge and taking it for a spin. However, in a real app you will almost always interact with Documents within an Automerge [Repository](/docs/concepts/#repositories). 
